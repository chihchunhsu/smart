...
parser.add_argument("-teff",type=float,
    default=None, help="effective temperature teff", nargs="+")

parser.add_argument("-logg",type=float,
    default=None, help="surface gravity logg", nargs="+")
############################################################
...
teff                   = float(args.teff[0])
logg                   = float(args.logg[0])
...
def lnlike(theta, data, lsf, pwv, teff, logg):
	"""
	...
	## Parameters MCMC
	vsini, rv, alpha, A, B, N = theta #N noise prefactor
...
def lnprior(theta, limits=limits):
	...
	vsini, rv, alpha, A, B, N = theta

	if limits['vsini_min'] < vsini < limits['vsini_max']\
...
def lnprob(theta, data, lsf, pwv, teff, logg):
	...
	return lnp + lnlike(theta, data, lsf, pwv, teff, logg)

pos = [np.array([	priors['vsini_min'] + (priors['vsini_max']  - priors['vsini_min']) * np.random.uniform(),
...
with Pool() as pool:
	#sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob, args=(data, lsf, teff, logg), a=moves, pool=pool)
	sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob, args=(data, lsf, pwv, teff, logg), a=moves, pool=pool,
...
ylabels = ["$vsin \, i(km/s)$","$RV(km/s)$","$\\alpha$","$C_{F_{\lambda}}$ (cnt/s)","$C_{\lambda}$($\AA$)","$C_{noise}$"]
...
vsini_mcmc, rv_mcmc, alpha_mcmc, A_mcmc, B_mcmc, N_mcmc = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]), 
...
plt.figtext(0.89,0.85,str(data.header['OBJECT'])+' '+data.name+' O'+str(data.order),
	...
	fontsize=15)
plt.figtext(0.89,0.82,"$Teff \, {0}/ logg \, {1}/ en \, 0.0/ vsini \, {2}^{{+{3}}}_{{-{4}}}/ RV \, {5}^{{+{6}}}_{{-{7}}}$".format(\
	round(teff),
	round(logg),
