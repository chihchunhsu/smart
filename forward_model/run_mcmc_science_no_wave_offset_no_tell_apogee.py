...
def lnlike(theta, data, lsf):
	...
	## Parameters MCMC
	teff, logg, vsini, rv, A, N = theta #A: flux offset; N: noise prefactor

	## wavelength offset is set to 0
	model = makeModel(teff, logg, 0.0, vsini, rv, 0.0, A,
...
ylabels = ["$Teff (K)$","$log \, g$","$vsin \, i \, (km/s)$","$rv \, (km/s)$","$C_{flux}$","$C_{noise}$"]
...
teff_mcmc, logg_mcmc, vsini_mcmc, rv_mcmc, A_mcmc, N_mcmc = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]), 
