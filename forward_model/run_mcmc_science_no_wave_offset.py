...
def makeModel(teff,logg,z,vsini,rv,alpha,wave_offset,flux_offset,**kwargs):
	...
	if data is not None:
		order = data.order
	# read in a model
	#model    = smart.Model(teff=teff, logg=logg, feh=z, order=order, modelset=modelset)
	model    = smart.Model()
	model.wave, model.flux = InterpModel(teff, logg, modelset=modelset, order=order)
	model.wave            *= 10000
	# wavelength offset
...
def InterpModel(Teff, Logg, modelset='btsettl08', order=33, T1=T1):
...
def lnlike(theta, data, lsf):
	...
	model = smart.makeModel(teff, logg, 0.0, vsini, rv, alpha, 0.0, A,
...
ylabels = ["$T_{eff}(K)$","$log \, g$","$vsin \, i(km/s)$","$RV(km/s)$","$\\alpha$","$C_{F_{\lambda}}$","$C_{noise}$"]
