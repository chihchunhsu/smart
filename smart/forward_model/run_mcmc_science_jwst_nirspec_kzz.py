...
elif 'barman' in modelset.lower():
	limits         = { 
						'teff_min':max(priors['teff_min']-300,800), 'teff_max':min(priors['teff_max']+300,1200),
						'logg_min':4.5,                             'logg_max':5.5,
						'kzz_min':1e2,                              'kzz_max':1e8,
						'vsini_min':0.0,                            'vsini_max':100.0,
						'rv_min':-1000.0,                           'rv_max':1000.0,
						'am_min':1.0,                               'am_max':3.0,
						'pwv_min':0.5,                            	'pwv_max':20.0,
...
def lnlike(theta, data, lsf0):
	...
	## Parameters MCMC
	#print('THETA:', theta)
	teff, logg, kzz, rv, A, N, lsf = theta #N noise prefactor
	#teff, logg, vsini, rv, , am, pwv, A, B, freq, amp, phase = theta

	model = model_fit.makeModel(teff=teff, logg=logg, metal=0.0, kzz=kzz, rv=rv, flux_mult=A, #wave_offset=B, 
		lsf=lsf, order=str(data.order), data=data, modelset=modelset, include_fringe_model=include_fringe_model, instrument=instrument, 
		tell=False)
	'''
	print('MODEL')
	print(model.wave)
	...
	ax1.plot(model.wave, model.flux, label='model', alpha=0.5)
	ax1.legend()
	ax2.plot(data.wave, data.flux, label='data', alpha=0.5)
	ax2.plot(model.wave, model.flux, label='model', alpha=0.5)
	ax2.set_xlim(44200, 45800)
	plt.tight_layout()
	#plt.show()
	plt.pause(0.1)
	plt.close()
...
#sampler_chain[:,:,2] = np.log10(sampler_chain[:,:,2])
...
teff_mcmc, logg_mcmc, kzz_mcmc, rv_mcmc, A_mcmc, N_mcmc, lsf_mcmc = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]), 
	zip(*np.percentile(triangle_samples, [16, 50, 84], axis=0)))


print(teff_mcmc, logg_mcmc, kzz_mcmc, rv_mcmc, A_mcmc, N_mcmc, lsf_mcmc)
...
model = model_fit.makeModel(teff=teff, logg=logg, metal=0.0, rv=rv, flux_mult=A, kzz=kzz, 
...
ax1.plot(data.wave, diff,'k-',alpha=0.8, label='residual')
#N=1
plt.fill_between(data.wave,-data.noise,data.noise,facecolor='C1',alpha=0.5)
...
#	fontsize=15)
plt.figtext(0.89,0.82,"$Teff \, {0}^{{+{1}}}_{{-{2}}}/ logg \, {3}^{{+{4}}}_{{-{5}}}/ log10(kzz) \, {6}^{{+{7}}}_{{-{8}}}/ RV \, {9}^{{+{10}}}_{{-{11}}}$".format(\
	round(teff_mcmc[0]),
	round(teff_mcmc[1]),
	round(teff_mcmc[2]),
	round(logg_mcmc[0],1),
