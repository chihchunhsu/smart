...
if modelset.lower() == 'btsettl08':
	...
	limits         = { 
						...
						'logg_min':3.,                             	'logg_max':logg_max,
						'z_min':-2.5,                             	'z_max':z_max,
						'vsini_min':0.0,                            'vsini_max':100.0,
						'rv_min':-200.0,                            'rv_max':200.0,
						'am_min':1.0,                               'am_max':3.0,
						'pwv_min':0.5,                            	'pwv_max':20.0,
...
def lnlike(theta, data, lsf):
	...
	## Parameters MCMC
	teff, logg, z, vsini, rv, am, pwv, A, B, N = theta #N noise prefactor
	#teff, logg, vsini, rv, , am, pwv, A, B, freq, amp, phase = theta

	model = model_fit.makeModel(teff=teff, logg=logg, metal=z, vsini=vsini, rv=rv, tell_alpha=1.0, wave_offset=B, flux_offset=A,
...
def lnprior(theta, limits=limits):
	...
	teff, logg, z, vsini, rv, am, pwv, A, B, N = theta
...
pos = [np.array([	priors['teff_min']  + (priors['teff_max']   - priors['teff_min'] ) * np.random.uniform(), 
					...
					priors['z_min']     + (priors['z_max']      - priors['z_min'] ) * np.random.uniform(), 
...
ylabels = ["$T_{eff} (K)$","$log \, g$(dex)","z","$vsin \, i(km/s)$","$RV(km/s)$","$AM$", "pwv (mm)","$C_{F_{\lambda}}$ (cnt/s)","$C_{\lambda}$($\AA$)","$C_{noise}$"]
...
teff_mcmc, logg_mcmc, z_mcmc, vsini_mcmc, rv_mcmc, am_mcmc, pwv_mcmc, A_mcmc, B_mcmc, N_mcmc = map(lambda v: (v[1], v[2]-v[1], v[1]-v[0]), 
...
file_log2.write("z_mcmc_e {}\n".format(str(max(abs(z_mcmc[1]), abs(z_mcmc[2])))))
...
file_log2.write("z_mcmc_ue {}\n".format(str(abs(z_mcmc[1]))))
...
file_log2.write("z_mcmc_le {}\n".format(str(abs(z_mcmc[2]))))
...
model, model_notell = model_fit.makeModel(teff=teff, logg=logg, metal=z, 
...
plt.figtext(0.89,0.85,str(data.header['OBJECT'])+' '+data.name+' O'+str(data.order),
	...
	fontsize=15)
plt.figtext(0.89,0.82,"$Teff \, {0}^{{+{1}}}_{{-{2}}}/ logg \, {3}^{{+{4}}}_{{-{5}}}/ z \, {6}^{{+{7}}}_{{-{8}}}/ en \, 0.0/ vsini \, {9}^{{+{10}}}_{{-{11}}}/ RV \, {12}^{{+{13}}}_{{-{14}}}$".format(\
	round(teff_mcmc[0]),
	round(teff_mcmc[1]),
	round(teff_mcmc[2]),
	round(logg_mcmc[0],1),
...
cat = cat.append({	'date_obs':date_obs,'date_name':sci_data_name,'tell_name':tell_data_name,
					...
					'z':z_mcmc[0], 'e_z':max(z_mcmc[1], z_mcmc[2]), 'ue_z':z_mcmc[1], 'le_z':z_mcmc[2],
