...
flux   = hdu[0].data
noise  = hdu2[0].data
wave   = 10**(header['CRVAL1']+header['CDELT1']*(np.arange(len(flux))+0))
...
def lnlike(theta, data, lsf):
	...
	model = model_fit.makeModel(teff=teff, logg=logg, metal=0.0, rv=rv, #wave_offset=B, 
		flux_mult=A, 
		lsf=lsf, order=str(data.order), data=data, modelset=modelset, include_fringe_model=False, instrument=instrument, tell=False, smooth=True)
	'''
	print('MODEL')
	print(model.wave)
	print(model.flux)
