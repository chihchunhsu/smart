...
print(sci_data_name)
print(data_path)
#data           = np.genfromtxt(data_path + sci_data_name + '.txt', skip_header=1)
hdulist       = fits.open(data_path + sci_data_name + '.fits', ignore_missing_end=True)
data  = hdulist[0].data

wave  = data[0] * 10000 * u.angstrom
flux  = data[1] * u.W/u.m**2/u.micron  ### NEED TO CONVERT THESE UNITS! erg/s / m^2 / micron
noise = data[2] * u.W/u.m**2/u.micron  ### NEED TO CONVERT THESE UNITS!
#ind   = np.where( (flux > 0) & (~np.isnan(flux) ) )
flux  = flux.to(u.erg/u.s/u.cm**2/u.angstrom)
noise = noise.to(u.erg/u.s/u.cm**2/u.angstrom)

ind   = np.where( (wave.value > 19000) & (~np.isnan(flux)) )
wave  = wave[ind]
flux  = flux[ind]
noise = noise[ind]
